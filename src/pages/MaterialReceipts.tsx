...
const MaterialReceipts = () => {
  const { records, loading, addRecord, updateRecord, deleteRecord } =
    useMaterialReceipts();
  const [selectedRecord, setSelectedRecord] = useState<MaterialReceipt | null>(
    null
  );
  ...
  const getQuantityValue = (record: MaterialReceipt) => {
    switch (record.unit_type) {
      case "KG":
        return record.net_weight || 0;
      case "M3":
        return record.volume_m3 || 0;
      case "M2":
        return record.volume_m2 || 0;
      case "LITROS":
        return record.volume_liters || 0;
      default:
        return 0;
    }
  };
  ...
  const totalMaterials = materialTypes
    .map((type) => ({
      type,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Controle de entrada de materiais e insumos
            </p>
          </div>
        </div>
        <Button
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                      <div className="flex-1 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge
                            variant={
                              record.exit_time ? "default" : "destructive"
                            }
                          >
                            {record.exit_time ? "Concluído" : "Pendente"}
                          </Badge>
                          <span className="text-sm text-muted-foreground">
                            {formatDateForDisplay(record.date)} às{" "}
                            {record.entry_time}
                          </span>
                        </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedRecord
                ...
                : "Registre o recebimento de material"}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Select
                name="material_type"
                defaultValue={selectedRecord?.material_type}
                required
              >
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  name="unit_type"
                  defaultValue={selectedRecord?.unit_type}
                  required
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="0.000"
                  defaultValue={
                    selectedRecord ? getQuantityValue(selectedRecord) : ""
                  }
                  required
                />
              </div>
