...
const Reports = () => {
  ...
  const { records: equipmentRecords, loading: loadingEquipment } =
    useEquipment();
  const { records: loadingRecords, loading: loadingLoadings } =
    useLoadingRecords();
  ...
  const thisMonthVehicles = vehicles.filter(
    (v) =>
      new Date(v.date).getMonth() === currentMonth &&
      new Date(v.date).getFullYear() === currentYear
  );

  const carregamentosPluma = thisMonthVehicles.filter(
    (v) =>
      v.type === "Carregamento" && v.purpose?.toLowerCase().includes("pluma")
  );

  const carregamentosCaroco = thisMonthVehicles.filter(
    (v) =>
  ...
  const exportToPDF = () => {
    ...
    if (loadingRecords.length > 0) {
      ...
      const loadingTableData = loadingRecords
        .slice(0, 20)
        .map((record) => [
          record.date,
          record.product,
    ...
    if (cottonRecords.length > 0 && yPosition < 250) {
      ...
      const cottonTableData = cottonRecords
        .slice(0, 15)
        .map((record) => [
          record.date,
          record.producer,
  ...
  const isLoading =
    loadingVehicles || loadingCotton || loadingRain || loadingEquipment;
  ...
  const generateDailySummary = (
    sendToWhatsAppFlag = false,
    showInModal = false
  ) => {
    const filterDateStr = dateFilter || today;
    const displayDate = new Date(
      filterDateStr + "T00:00:00"
    ).toLocaleDateString("pt-BR");

    // Chuva do dia e do mês
    const chuvaHoje = rainRecords
      .filter((r) => r.date === filterDateStr && r.millimeters !== null)
      .reduce((sum, r) => sum + (r.millimeters || 0), 0);
    const monthStart = new Date(
      new Date(filterDateStr).getFullYear(),
      new Date(filterDateStr).getMonth(),
      1
    )
      .toISOString()
      .split("T")[0];
    const chuvaMes = rainRecords
      .filter(
        (r) =>
          r.date >= monthStart &&
          r.date <= filterDateStr &&
          r.millimeters !== null
      )
      .reduce((sum, r) => sum + (r.millimeters || 0), 0);

    // Carregamentos do dia (entry_date = data filtrada)
    const carregamentosConcluidos = loadingRecords.filter(
      (l) =>
        convertIsoToLocalDateString(l.exit_date) === filterDateStr ||
        (l.status === "carregado" &&
          convertIsoToLocalDateString(l.loaded_at) === filterDateStr)
    );

    // Agrupar carregamentos por produto
    const plumaCarregamentos = carregamentosConcluidos.filter(
      (l) => l.product === "Pluma"
    );
    const carocoCarregamentos = carregamentosConcluidos.filter(
      (l) => l.product === "Caroço"
    );
    const fibrilhaCarregamentos = carregamentosConcluidos.filter(
      (l) => l.product === "Fibrilha"
    );
    const briqueteCarregamentos = carregamentosConcluidos.filter(
      (l) => l.product === "Briquete"
    );

    // Calcular totais
    const totalPlumaFardos = plumaCarregamentos.reduce(
      (sum, l) => sum + (l.bales || 0),
      0
    );
    const totalCarocoKg = carocoCarregamentos.reduce(
      (sum, l) => sum + (l.weight || 0),
      0
    );
    const totalFibrilhaFardos = fibrilhaCarregamentos.reduce(
      (sum, l) => sum + (l.bales || 0),
      0
    );
    const totalBriqueteKg = briqueteCarregamentos.reduce(
      (sum, l) => sum + (l.weight || 0),
      0
    );
    ...
    // Materiais recebidos
    const todayMaterials = materialRecords.filter(
      (m) => m.date === filterDateStr
    );

    // Saída de equipamentos do dia
    const todayEquipment = equipmentRecords.filter(
      (eq) => eq.date === filterDateStr
    );

    // Fila de carregamento atual (apenas na fila)
    const filaAtual = loadingRecords.filter((l) => !l.entry_date);
    const filaPlumaCount = filaAtual.filter(
      (l) => l.product === "Pluma"
    ).length;
    const filaCarocoCount = filaAtual.filter(
      (l) => l.product === "Caroço"
    ).length;
    const filaFibrilhaCount = filaAtual.filter(
      (l) => l.product === "Fibrilha"
    ).length;
    const filaBriqueteCount = filaAtual.filter(
      (l) => l.product === "Briquete"
    ).length;
...
🚛 CARREGAMENTOS CONCLUÍDOS:`;
    ...
    if (plumaCarregamentos.length > 0) {
      message += `\n▪️ Pluma: ${
        plumaCarregamentos.length
      } caminhões | ${totalPlumaFardos.toLocaleString("pt-BR")} fardos`;
    }
    if (carocoCarregamentos.length > 0) {
      message += `\n🌰 Caroço: ${
        carocoCarregamentos.length
      } caminhões | ${totalCarocoKg.toLocaleString("pt-BR")} kg`;
    }
    if (fibrilhaCarregamentos.length > 0) {
      message += `\n▪️ Fibrilha: ${
        fibrilhaCarregamentos.length
      } caminhões | ${totalFibrilhaFardos.toLocaleString("pt-BR")} fardos`;
    }
    if (briqueteCarregamentos.length > 0) {
      message += `\n🔥 Briquete: ${
        briqueteCarregamentos.length
      } caminhões | ${totalBriqueteKg.toLocaleString("pt-BR")} kg`;
    }
    ...
    if (todayMaterials.length > 0) {
      ...
      const cavacoMaterials = todayMaterials.filter(
        (m) => m.material_type === "Cavaco"
      );
      const otherMaterials = todayMaterials.filter(
        (m) => m.material_type !== "Cavaco"
      );

      // Cavaco: mostrar apenas quantidade de cargas e peso total
      if (cavacoMaterials.length > 0) {
        const totalCavacoWeight = cavacoMaterials.reduce(
          (sum, m) => sum + m.net_weight,
          0
        );
        message += `\n• Cavaco: ${cavacoMaterials.length} carga${
          cavacoMaterials.length > 1 ? "s" : ""
        } | ${totalCavacoWeight.toLocaleString("pt-BR")} kg`;
      }

      // Outros materiais: agrupar por tipo e fornecedor
      if (otherMaterials.length > 0) {
        const groupedMaterials = otherMaterials.reduce((acc, material) => {
          const key = `${material.material_type}|${
            material.supplier || "Fornecedor não informado"
          }`;
          if (!acc[key]) {
            acc[key] = {
          ...
          if (material.unit_type === "KG") {
            ...
            material.unit_type === "LITROS" &&
            material.volume_liters
          ) {
        ...
        const materialsByType = Object.values(groupedMaterials).reduce(
          (acc, item) => {
          ...
          {} as Record<
            string,
            { total: number; unit: string; suppliers: Set<string> }
          >
        );
        ...
        Object.entries(materialsByType).forEach(([materialType, data]) => {
          if (data.suppliers.size === 1) {
            ...
            message += `\n• ${materialType}: ${data.total.toLocaleString(
              "pt-BR"
            )} ${data.unit} de ${supplier}`;
          } else {
            // Múltiplos fornecedores - mostrar cada um
            const itemsForType = Object.values(groupedMaterials).filter(
              (item) => item.material_type === materialType
            );
            itemsForType.forEach((item) => {
              message += `\n• ${
                item.material_type
              }: ${item.quantity.toLocaleString("pt-BR")} ${item.unit} de ${
                item.supplier
              }`;
            });
          }
  ...
  const generateQueueStatus = (
    ...
    const filaCarregamento = loadingRecords.filter(
      (l) => !l.entry_date && (!dateFilter || l.date === filterDateStr)
    );

    // Agrupar por produto
    const filaPluma = filaCarregamento.filter((l) => l.product === "Pluma");
    const filaCaroco = filaCarregamento.filter((l) => l.product === "Caroço");
    const filaFibrilha = filaCarregamento.filter(
      (l) => l.product === "Fibrilha"
    );
    const filaBriquete = filaCarregamento.filter(
      (l) => l.product === "Briquete"
    );
    const filaOutros = filaCarregamento.filter(
      (l) => !["Pluma", "Caroço", "Fibrilha", "Briquete"].includes(l.product)
    );
...
📊 TOTAL: ${totalCarretas} carreta${
      totalCarretas !== 1 ? "s" : ""
    } aguardando\n\n`;

    // Função auxiliar para agrupar e formatar por produto
    const formatProductQueue = (
      items: typeof filaCarregamento,
      productName: string,
      icon: string
    ) => {
      if (items.length === 0) return "";

      let result = `${icon} ${productName} - ${items.length} Carreta${
        items.length > 1 ? "s" : ""
      }\n`;
      ...
      Object.entries(byCarrier)
        ...
        .forEach(([carrier, carrierItems]) => {
          ...
          const byTruckType = carrierItems.reduce((acc, item) => {
            const key = `${item.truck_type}|${
              item.is_sider ? "Sider" : "Normal"
            }`;
            if (!acc[key]) {
              acc[key] = {
                truck_type: item.truck_type,
                is_sider: item.is_sider,
                count: 0,
                siderCount: 0,
              };
            }
          ...
          Object.values(byTruckType).forEach((group) => {
            const siderInfo =
              group.siderCount > 0 ? ` (${group.siderCount} Sider)` : "";
    ...
    if (filaOutros.length > 0) {
      message += `📦 Outros Produtos – ${filaOutros.length} carreta${
        filaOutros.length > 1 ? "s" : ""
      } aguardando:\n`;
  ...
  const generateCottonPullSummary = (
    ...
    const todayRecords = cottonRecords.filter(
      (record) => record.date === filterDateStr
    );
    ...
    const groupedData = todayRecords.reduce((acc, record) => {
      ...
      if (
        !acc[key].lastExit ||
        (record.exit_time && record.exit_time > acc[key].lastExit)
      ) {
...
`;
    ...
    const recordsWithBothTimes = todayRecords.filter(
    ...
    const avgAlgodoeiraTime =
      recordsWithBothTimes.length > 0
        ? Math.round(totalAlgodoeiraTime / recordsWithBothTimes.length)
        : 0;
    const algodoeiraHours = Math.floor(avgAlgodoeiraTime / 60);
    const algodoeiraMinutes = avgAlgodoeiraTime % 60;
    const algodoeiraTimeStr =
      algodoeiraHours > 0
        ? `${algodoeiraHours}h ${algodoeiraMinutes}min`
        : `${algodoeiraMinutes}min`;

    // Calcular média de tempo de viagem do dia (todas as viagens)
    const totalTimeAllVehicles = vehiclesArray.reduce(
      (sum, v) => sum + v.totalTime,
      0
    );
    const avgTripTime =
      totalTrips > 0 ? Math.round(totalTimeAllVehicles / totalTrips) : 0;
      ...
      navigator.clipboard
        ...
        .then(() => {
          toast({
            title: "Copiado!",
            description:
              "Relatório de Puxe de Rolos copiado para área de transferência.",
          });
        })
        .catch(() => {
          toast({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-accent/5">
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
                      ...
                      {periodFilter === "day"
                        ...
                        ? cottonRecords
                            .filter(
                              (r) =>
                                !dateFilter ||
                                r.date.startsWith(dateFilter.substring(0, 7))
                            )
                            .reduce((sum, r) => sum + r.rolls, 0)
                            .toLocaleString("pt-BR")
                        : cottonRecords
                ...
                <div className="mb-4 p-3 bg-orange-100 rounded border-2 border-orange-600">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-semibold text-orange-900">
                      {showAllVehicles
                        ? `Mostrando ${loadingRecords.length} registros`
                        : `Mostrando registros filtrados`}
                    </span>
                  </div>
                </div>
                    ...
                    {loadingRecords
                      .filter((l) => {
                        ...
                        const matchesColumns =
                          ...
                          (!columnFilters.driver ||
                            l.driver
                              ?.toLowerCase()
                              .includes(columnFilters.driver.toLowerCase())) &&
                          (!columnFilters.carrier ||
                            l.carrier
                              ?.toLowerCase()
                              .includes(columnFilters.carrier.toLowerCase())) &&
                          (!columnFilters.destination ||
                            l.destination
                              ?.toLowerCase()
                              .includes(
                                columnFilters.destination.toLowerCase()
                              ));
                      ...
                      .map((loading) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="border border-gray-300 p-2 text-sm">
                              {toTitleCase(loading.driver)}
                            </td>
                            <td className="border border-gray-300 p-2 text-sm">
                              {toTitleCase(loading.carrier)}
                            </td>
                            <td className="border border-gray-300 p-2 text-sm">
                              {toTitleCase(loading.destination)}
                            </td>
                            <td className="border border-gray-300 p-2 text-sm">
            ...
            {!isExpanded && (
              <div className="text-center py-8 text-muted-foreground">
                <p className="text-sm mb-2">
                  Clique em "Expandir Tudo" para ver a tabela completa de
                  movimentações
                </p>
                <p className="text-xs">
                  Total de {loadingRecords.length} registros de carregamento
                  disponíveis
                </p>
              </div>
            )}
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Ranking de motoristas por rolos puxados
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {(() => {
                  ...
                  return sortedDrivers.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        Cadastre registros para ver o ranking
                      </p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {sortedDrivers.map((driver, index) => (
                            <tr
                              key={`${driver.driver}-${driver.plate}`}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Distribuição de carretas no período
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {loadingByTruck.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">
                      Cadastre veículos para ver a distribuição por tipo
                    </p>
                  </div>
                ) : (
                  loadingByTruck.map((item, index) => (
                    <div key={index} className="flex items-center gap-4">
                        ...
                        <div className="w-full bg-muted rounded-full h-3">
                          <div
                            ...
                            style={{
                              width: `${
                                loadingByTruck[0]
                                  ? (item.count / loadingByTruck[0].count) * 100
                                  : 0
                              }%`,
                            }}
                          />
                      ...
                      <p className="font-bold text-accent w-12 text-right">
                        {item.count}
                      </p>
                    </div>
                  ))
        ...
        {(() => {
          const materialsByType = materialRecords.reduce(
            (
              acc: Record<
                string,
                {
              ...
              if (material.unitType === "KG") {
                ...
                material.unitType === "LITROS" &&
                material.volume_liters
              ) {
                acc[material.material_type].totalLitros +=
                  material.volume_liters;
              }
            ...
            <Card>
              <CardContent className="py-8">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Registre materiais para ver os resumos aqui
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Gere mensagens automáticas para compartilhar
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Filtro de Data para Relatórios */}
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <Label htmlFor="reportDateFilter" className="font-semibold">
      ...
      >
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Visualize a mensagem abaixo e clique em "Copiar" para copiar para
