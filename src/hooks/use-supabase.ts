...
export const useVehicles = () => {
  ...
  const updateVehicle = async (id: string, updates: Partial<Vehicle>) => {
    // Normaliza a data para YYYY-MM-DD local para evitar deslocamento por UTC
    if ((updates as any).date) {
      (updates as any).date = normalizeLocalDate((updates as any).date);
    }
    try {
      const { data, error } = await supabase
        .from("vehicles")
...
export const useCottonPull = () => {
  ...
  const addRecord = async (
    recordData: Omit<CottonPull, "id" | "created_at" | "updated_at">
  ) => {
    try {
      const { data, error } = await supabase
        .from("cotton_pull")
...
export const useRainRecords = () => {
  ...
  const addRecord = async (
    recordData: Omit<RainRecord, "id" | "created_at" | "updated_at">
  ) => {
    try {
      const { data, error } = await supabase
        .from("rain_records")
...
export const useEquipment = () => {
  ...
  const addRecord = async (
    recordData: Omit<Equipment, "id" | "created_at" | "updated_at">
  ) => {
    try {
      const { data, error } = await supabase
        .from("equipment")
...
export const useLoadingRecords = () => {
  ...
  const initializeTable = useCallback(async () => {
    try {
      ...
      if (
        error &&
        error.message.includes('relation "loading_records" does not exist')
      ) {
        ...
        toast({
          title: "Aviso",
          description:
            "Sistema em modo simulação - dados não serão persistidos.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const addRecord = async (
    recordData: Omit<LoadingRecord, "id" | "created_at" | "updated_at">
  ) => {
    try {
      ...
      toast({
        title: "Erro",
        description: `Erro: ${
          error?.message || "Não foi possível registrar o carregamento"
        }`,
        variant: "destructive",
      });
      throw error;
...
export const usePuxeViagens = () => {
  ...
  const addViagem = async (
    viagemData: Omit<PuxeViagem, "id" | "created_at" | "updated_at">
  ) => {
    try {
      const { data, error } = await supabase
        .from("puxe_viagens")
